 ...
 */
function handleDbConstraintError(error: unknown): { message: string; statusCode: number } | null {
  // Drizzle wraps postgres.js errors in DrizzleQueryError with the original PG error at error.cause
...
function mapErrorToResponse(error: unknown): {
  ...
  if (error && typeof error === 'object' && 'message' in error) {
    const statusCode =
      'status' in error ? (error.status as number) : HttpStatusCodes.INTERNAL_SERVER_ERROR;
  ...
  const statusCode =
    currentStatus !== HttpStatusCodes.OK ? currentStatus : HttpStatusCodes.INTERNAL_SERVER_ERROR;
...
export const errorHandler = async (c: AppContext, next: Next) => {
  try {
    ...
    return c.json(
      errorResponse(statusCode, message, appConfig.isDevelopment ? details : undefined),

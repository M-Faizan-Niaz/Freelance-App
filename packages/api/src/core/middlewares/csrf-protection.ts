...
export const setCsrfToken = async (c: Context, next: Next) => {
  ...
  if (c.req.method === 'GET') {
    ...
    const existingToken = await getSignedCookie(c, CSRF_SECRET, CSRF_COOKIE_NAME);

    if (!existingToken) {
      const token = generateCsrfToken();

      // Sign the cookie with dedicated CSRF secret
      await setSignedCookie(c, CSRF_COOKIE_NAME, token, CSRF_SECRET, CSRF_COOKIE_OPTIONS);

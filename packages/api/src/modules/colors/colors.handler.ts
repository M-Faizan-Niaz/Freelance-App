...
import type { CreateColorsResponse, GetColorsResponse, ListColorsResponse } from './colors.schema';
...
export const getOne: AppRouteHandler<GetOneRoute> = async (c) => {
  ...
  const response = successResponse(typedResponse, 'STANDARD_MESSAGES.SUCCESS.FETCHED');
  return c.json(response, HttpStatusCodes.OK);
};
...
export const patch: AppRouteHandler<PatchRoute> = async (c) => {
  ...
  const response = successResponse(updatedColors, 'Colors updated successfully');
  return c.json(response, HttpStatusCodes.OK);
};
...
export const removeSelected: AppRouteHandler<RemoveSelectedRoute> = async (c) => {
  ...
  await colorsService.deleteColors(ids);

...
import type { NewColors } from './colors.model';
...
export class ColorsRepository {
  ...
  async list(params: {
    ...
    const searchCondition = createSearchCondition(searchableFields, colors, search);
   ...
   */
  async update(tx: TX, id: number, colorsData: UpdateColorsRequest) {
    const [result] = await tx
      .update(colors)
      .set({
        ...colorsData,

...
export const DEFAULT_PASSWORD_OPTIONS: PasswordValidationOptions = {
  ...
  bannedPasswords: ['password', '123456', '123456789', 'qwerty', 'admin', 'welcome', 'letmein'],
};
 ...
 */
function createBaseLengthSchema(options: PasswordValidationOptions): z.ZodString {
  return z
    .string()
    .min(options.minLength ?? 1, `Password must be at least ${options.minLength} characters`)
    .max(options.maxLength ?? 72, `Password must be at most ${options.maxLength} characters`);
}
...
function addComplexityRequirements(
  ...
  if (options.requireAllGroups) {
    return addAllGroupsRequirements(schema, patterns, options, activeRequirements);
  } else {
    return addAnyGroupRequirements(schema, patterns, options, activeRequirements);
  }
}
...
function addAllGroupsRequirements(
  ...
  if (options.requireNumbers) {
    result = result.regex(patterns.numbers, 'Password must contain at least one number');
...
function addAnyGroupRequirements(
  ...
  if (activePatterns.length > 0) {
    const combinedPattern = new RegExp(activePatterns.map((p) => `(?=.*${p.source})`).join(''));
 ...
 */
function addBannedPasswordCheck(schema: z.ZodString, bannedPasswords: string[]): z.ZodType<string> {

...
export const createSuccessResponseSchema = <T extends ZodSchema>(
  ...
  const extendedSchema = schema as any; // Cast to access .openapi() method
  ...
  return z
    .object({
      ...
      data:
        typeof extendedSchema.openapi === 'function'
          ? extendedSchema.openapi({ example: example ?? undefined })
          : extendedSchema,
      meta: z
        .looseObject({

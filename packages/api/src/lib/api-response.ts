 ...
 */
export interface ApiSuccessResponseWithPagination<T = unknown> extends ApiSuccessResponse<T> {
  meta: {
    pagination: PaginationSchema;
 ...
 */
export function errorResponse(code: number, message: string, details?: unknown): ApiErrorResponse {
  return {
    success: false,
    message,
    error: {
